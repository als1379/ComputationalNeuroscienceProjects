...
class LIF:
    ...
    def __init__(self, I: Callable[[int], float],
                 ...
                 neuron_type: Optional[NeuronType] = NeuronType.Excitatory,
                 R: Optional[int] = 10,
                 dt: Optional[float] = 0.03125,
                 u_rest: Optional[int] = -79,
                 u_reset: Optional[int] = -70,
                 u_spike: Optional[int] = 5,
                 treshold: Optional[int] = -40,
                 tau: Optional[int] = 8,
        ...
        # handle connections effects in populations
        self.pre_synapses_effect = defaultdict(lambda: 0, {})
        self.post_synapses = []
        self.connections = [] #STDP
    ...
    def execute(self, n_iter: int) -> None:
        ...
        while self.iter <= n_iter:
            delta_u, new_w = self.strategy.calculate_delta(
                                                    ...
                                                    u_rest=self.u_rest,
                                                    R=self.R,
                                                    I=self.I(self.iter * self.dt) + self.I_inp, # SNN
                                                    tau=self.tau,
                                                    dt=self.dt,
            ...
            # add effects of previous connected neurons
            u_new += self.pre_synapses_effect[self.iter]

            if u_new >= self.treshold:
                self.u.append(self.treshold + self.u_spike)
                self.fires.append(self.iter)
                self.iter += 1
                u_new = self.u_reset + self.pre_synapses_effect[self.iter]
